...
# Append CHANGELOG entry (prepend-style)
STAMP_UTC="$(date -u '+%Y-%m-%d %H:%M:%S UTC')"
ENTRY=$'# '"$TAG"$' — '"$STAMP_UTC"$'\n- Build: '"$NEW_BUILD"$'\n- Env: '"$ENV_NAME"$' ('"$HOST"$')\n'
mkdir -p docs
if [[ -f docs/CHANGELOG.md ]]; then
  { echo "$ENTRY"; echo; cat docs/CHANGELOG.md; } > docs/CHANGELOG.md.tmp && mv docs/CHANGELOG.md.tmp docs/CHANGELOG.md
else
  { echo "# Changelog"; echo; echo "$ENTRY"; } > docs/CHANGELOG.md
fi

git add docs/CHANGELOG.md
git commit -m "chore: changelog entry $TAG"
git push

# Optional: detailed verifier
